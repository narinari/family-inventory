...
describe('families/{familyId}/boxes', () => {
  ...
  describe('read', () => {
    it('should deny unauthenticated access', async () => {
      await withSecurityRulesDisabled(async (context) => {
        const db = context.firestore();
        await setDoc(
          doc(db, 'families', familyId, 'boxes', 'box-1'),
          createTestBox()
        );
      });
      ...
      await assertFails(
        getDoc(doc(unauthDb, 'families', familyId, 'boxes', 'box-1'))
      );
    });
    ...
    it('should allow family member to read', async () => {
      ...
      await assertSucceeds(
        getDoc(doc(authDb, 'families', familyId, 'boxes', 'box-1'))
      );
    });
    ...
    it('should deny access to other family data', async () => {
      await withSecurityRulesDisabled(async (context) => {
        const db = context.firestore();
        await setDoc(
          doc(db, 'families', otherFamilyId, 'boxes', 'box-1'),
          createTestBox()
        );
      });
      ...
      const authDb = getAuthContext(memberId).firestore();
      await assertFails(
        getDoc(doc(authDb, 'families', otherFamilyId, 'boxes', 'box-1'))
      );
    });
  });
  ...
  describe('create', () => {
    it('should deny unauthenticated create', async () => {
      ...
      await assertFails(
        setDoc(
          doc(unauthDb, 'families', familyId, 'boxes', 'box-1'),
          createTestBox()
        )
      );
    });
    ...
    it('should allow family member to create with required fields', async () => {
      ...
      await assertSucceeds(
        setDoc(
          doc(authDb, 'families', familyId, 'boxes', 'box-1'),
          createTestBox()
        )
      );
    });
    ...
    it('should deny create without required name field', async () => {
      const authDb = getAuthContext(memberId).firestore();
      await assertFails(
        setDoc(doc(authDb, 'families', familyId, 'boxes', 'box-1'), {
    ...
    it('should deny create in other family', async () => {
      ...
      await assertFails(
        setDoc(
          doc(authDb, 'families', otherFamilyId, 'boxes', 'box-1'),
          createTestBox()
        )
      );
    });
  ...
  describe('update', () => {
    ...
    it('should deny unauthenticated update', async () => {
      const unauthDb = getUnauthContext().firestore();
      await assertFails(
        updateDoc(doc(unauthDb, 'families', familyId, 'boxes', 'box-1'), {
          name: 'Updated Box',
        })
      );
    });
    ...
    it('should allow family member to update', async () => {
      const authDb = getAuthContext(memberId).firestore();
      await assertSucceeds(
        updateDoc(doc(authDb, 'families', familyId, 'boxes', 'box-1'), {
          name: 'Updated Box',
        })
      );
    });
    ...
    it('should deny update in other family', async () => {
      ...
      const authDb = getAuthContext(memberId).firestore();
      await assertFails(
        updateDoc(doc(authDb, 'families', otherFamilyId, 'boxes', 'box-1'), {
          name: 'Updated Box',
        })
      );
    });
  ...
  describe('delete', () => {
    ...
    it('should deny unauthenticated delete', async () => {
      const unauthDb = getUnauthContext().firestore();
      await assertFails(
        deleteDoc(doc(unauthDb, 'families', familyId, 'boxes', 'box-1'))
      );
    });
    ...
    it('should deny member delete', async () => {
      const authDb = getAuthContext(memberId).firestore();
      await assertFails(
        deleteDoc(doc(authDb, 'families', familyId, 'boxes', 'box-1'))
      );
    });
    ...
    it('should deny admin delete in other family', async () => {
      ...
      const authDb = getAuthContext(adminId).firestore();
      await assertFails(
        deleteDoc(doc(authDb, 'families', otherFamilyId, 'boxes', 'box-1'))
      );
    });
  });
});
