...
#### 詳細タスク
- [ ] `events/messageReactionAdd.ts` イベントハンドラ作成
- [ ] 🤖絵文字のリアクション検知
- [ ] リアクション対象メッセージの取得
- [ ] 既存NLP処理（`lib/nlp.ts`）への連携
- [ ] 処理結果の返信（リアクションしたメッセージへのリプライ）
- [ ] Bot自身のメッセージへのリアクションは無視
- [ ] 部分メッセージのフェッチ対応（キャッシュにない場合）
- [ ] `src/index.ts` にイベント登録追加
