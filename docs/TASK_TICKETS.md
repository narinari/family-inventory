...
#### 対象ファイル
- `routes/auth.ts` (506行 → 328行)
- `services/auth.service.ts` (231行) - 変更なし

#### 詳細タスク
- [x] auth.ts に TASK-R01 のヘルパー適用
- [ ] Discord OAuth 処理（約100行）を `discord-oauth.ts` に分離（検討・後続タスク）
- [x] テスト確認: `pnpm --filter api test -- discord-oauth`
...
#### 対象ファイル
- `routes/wishlist.ts` (264行 → 168行)
- `services/wishlist.service.ts` (227行) - 変更なし

#### 詳細タスク
- [x] wishlist.ts に TASK-R01 のヘルパー適用
- [x] テスト確認: `pnpm --filter api test -- wishlist`
...
#### 対象ファイル
- `routes/items.ts` (450行 → 283行)
- `services/item.service.ts` (487行) - 変更なし

#### 詳細タスク
- [x] items.ts に TASK-R01 のヘルパー適用
- [ ] item.service.ts の機能分割検討（後続タスク）
- [ ] `getBoxesCollection`, `getLocationsCollection` の共通モジュール化（後続タスク）
- [x] テスト確認: `pnpm --filter api test -- items`
