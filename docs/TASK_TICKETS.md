...
#### 詳細タスク
- [ ] `firestore.rules`: itemTypes の delete ルールを変更
- [ ] テスト更新: Firestoreルールテストの削除テストケース修正
- [ ] 動作確認
...
#### 詳細タスク
- [ ] Web: アイテム詳細画面に「同じものを追加」ボタン追加
- [ ] Web: 新規作成画面でクエリパラメータからプリセット値を受け取る
- [ ] Web: プリセット値（種別、タグ、保管場所等）をフォームに反映
...
#### 詳細タスク
- [ ] Web: `/item-types/detail/page.tsx` 新規作成
- [ ] API: `GET /item-types/:id` 単体取得エンドポイント追加
- [ ] Web: アイテム詳細画面のリンク先を変更
- [ ] Web: 種別情報（名前、メーカー、説明、タグ）を表示
- [ ] Web: この種別のアイテム数と一覧へのリンク
- [ ] Web: 編集・削除機能
...
#### 詳細タスク
- [ ] マークダウンレンダリングコンポーネント作成
- [ ] アイテム詳細画面: メモ表示をマークダウン対応
- [ ] 箱詳細画面: メモ表示をマークダウン対応
- [ ] 保管場所詳細画面: メモ表示をマークダウン対応
- [ ] 購入予定詳細画面: メモ表示をマークダウン対応
- [ ] アイテム種別詳細画面: 説明表示をマークダウン対応
